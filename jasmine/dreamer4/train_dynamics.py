...
class Args:
    ...
    mlp_ratio: int = 4
    dropout: float = 0.0
    # Minecraft action space (CameraHierarchicalActionMapping)
    n_camera_bins: int = 11   # bins per camera axis (must be odd)
    ...
    entity: str = "4bkang"
    project: str = "jasmine"
    name: str = "dynamics_dreamer4_minecraft"
...
def build_model(args: Args, rngs: nnx.Rngs) -> tuple[TokenizerDreamer4, DynamicsDreamer4]:
    ...
    print(f"Action space: n_buttons={action_mapper.n_buttons}, n_camera={action_mapper.n_camera}")
...
def build_mesh_and_sharding(
    ...
    # Minecraft actions are hierarchical (B, T, 2): [button_idx, camera_idx]
