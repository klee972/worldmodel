"""
Standalone validation script for CoinRun dynamics model.
...
class Args:
    ...
    # Checkpoint
    ckpt_dir: str = "ckpts/coinrun/dreamer4/dynamics_ctx1"
    restore_step: int = 40000  # 0 = latest
    # Validation data
    ...
    entity: str = "4bkang"
    project: str = "jasmine"
    name: str = "val_dynamics_dreamer4_coinrun"
...
def build_val_dataloader(args: Args) -> grain.DataLoaderIterator:
    ...
    array_record_files = [
        os.path.join(args.val_data_dir, x)
        for x in os.listdir(args.val_data_dir)
...
def main(args: Args) -> None:
    ...
    # Dummy optimizer needed to match checkpoint state structure
    lr_schedule = get_lr_schedule("wsd", 0.0, 3e-4, 0.0, 100_000, 5000, 20_000)
    ...
    if args.log and jax.process_index() == 0:
        ...
        for tag in val_tags:
            ...
            for b in range(N):
                ...
                log_media[f"val/{tag}_video/{b}"] = wandb.Video(side_by_side, fps=10, format="mp4")
